...
const UI_LABELS={
  blockType:{definition:'Определение',structure:'Структура',table:'Таблица',keyPoint:'Ключевой момент',diagram:'Схема'},
  verification:{SourceChecked:'Источник проверен','legacy-unverified':'Не проверено'},
  reviewStatus:{DUE:'К повторению',SCHEDULED:'Запланировано',COMPLETED:'Завершено'},
  objectStatus:{active:'Активно',hidden:'Скрыто',archived:'В архиве',trashed:'Удалено'},
  progress:{WEAK:'Слабая',REVIEWED:'Изучено',MASTERED:'Изучено',IN_PROGRESS:'В процессе',NOT_STARTED:'Не начато'}
};
const uiBlockType=v=>UI_LABELS.blockType[v]||v||'';
const uiVerification=v=>UI_LABELS.verification[v]||'Статус не указан';
const uiReviewStatus=v=>UI_LABELS.reviewStatus[v]||v||'';
const uiObjectStatus=v=>UI_LABELS.objectStatus[v]||v||'';
const isTopicReady=t=>!!t&&state.knowledgeData.contentBlocks.some(b=>b.topicId===t.id)&&state.knowledgeData.questions.some(q=>q.topicId===t.id);
function renderBlockContent(b){
  const text=String(b.content||'');
  if(b.type==='diagram'){const parts=text.split(' → ');return `<div class="flow-steps">${parts.map((x,i)=>`<div class="flow-step"><span class="flow-num">${i+1}</span><span>${esc(x)}</span></div>`).join('')}</div>`;}
  const sentences=text.match(/[^.]+\.?/g)?.map(x=>x.trim()).filter(Boolean)||[text];
  if(b.type==='table'||b.type==='structure')return `<div class="fact-grid">${sentences.map(x=>`<div class="fact-row">${esc(x)}</div>`).join('')}</div>`;
  if(b.type==='keyPoint')return `<ul class="readable-list">${sentences.map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`;
  return `<div class="readable-stack">${sentences.map(x=>`<p>${esc(x)}</p>`).join('')}</div>`;
}
function shell(body){
  appEl.innerHTML=`<div class="app"><div class="top"><div class="brand"><div class="logo">✚</div><div><b>MedWiz ${APP_VERSION}</b><div class="qa">Сборка QA • не релиз</div></div></div><div class="row"><button class="btn ghost" id="searchTop">⌕</button><button class="btn ghost" id="manageTop">⚙︎</button><button class="btn ghost" id="themeBtn">${state.userData.theme==='dark'?'☀️':'🌙'}</button></div></div>${body}<div class="bottom"><button class="nav ${page==='home'?'active':''}" data-page="home">⌂<br>Главная</button><button class="nav ${['knowledge','subject','section','topic'].includes(page)?'active':''}" data-page="knowledge">▦<br>Знания</button><button class="nav ${page==='review'?'active':''}" data-page="review">↻<br>Повторение</button><button class="nav ${page==='exam'?'active':''}" data-page="exam">✓<br>Экзамен</button></div></div>`;
...
function openTopic(id){const t=topicBy(id);if(!t)return;if(!isTopicReady(t)){alert('Материал этой темы пока готовится. Полный сценарий обучения станет доступен после добавления утверждённого контента и теста.');return;}currentTopicId=id;currentSectionId=t.sectionId;currentSubjectId=sectionBy(t.sectionId)?.subjectId||null;state.userData.lastTopicId=id;save();page='topic';render()}
...
function home(){
  ...
  ${last?`<h2>Продолжить</h2><div class="item ${isTopicReady(last)?'':'disabled-topic'}" ${isTopicReady(last)?`data-open-topic="${last.id}"`:''}><div><b>${esc(last.name)}</b><div class="muted">${esc(sectionBy(last.sectionId)?.name||'')}</div>${isTopicReady(last)?'':'<div class="coming-soon">Контент готовится</div>'}</div><span>${isTopicReady(last)?'›':'Недоступно'}</span></div>`:''}
  ...
  <h2>Слабые темы</h2><div class="list">${weak.length?weak.map(t=>`<div class="item" data-open-topic="${t.id}"><span>${esc(t.name)}</span><span class="pill weak">Слабая</span></div>`).join(''):'<div class="notice">Пока слабых тем нет.</div>'}</div>
  <h2>Моя библиотека</h2><div class="grid two"><button class="card library-card" id="openFavorites"><b>★ Избранное</b><span class="muted">${state.learningData.favorites.length} тем</span></button><button class="card library-card" id="openNotes"><b>✎ Заметки</b><span class="muted">${Object.values(state.learningData.notes).filter(v=>String(v||'').trim()).length} записей</span></button></div>`);
...
function sectionPage(){const sec=sectionBy(currentSectionId),s=subjectBy(sec?.subjectId);if(!sec){page='knowledge';return render()} shell(`<div class="breadcrumb">Знания → ${esc(s?.name||'')} → ${esc(sec.name)}</div><button class="btn ghost" id="back">← ${esc(s?.name||'Предмет')}</button><div class="card" style="margin-top:12px"><h1>${esc(sec.name)}</h1><p class="muted">${esc(sec.description||'')}</p></div><h2>Темы</h2><div class="list">${topicsFor(sec.id).filter(t=>t.status==='active').map(t=>{let p=progressFor(t.id),ready=isTopicReady(t);return `<div class="item ${ready?'':'disabled-topic'}" ${ready?`data-topic="${t.id}"`:''}><div><b>${esc(t.name)}</b><div class="muted">${esc(t.summary||'')}</div>${ready?`<div class="progress" style="width:180px;margin-top:8px"><i style="width:${Number(p.lastScore||0)}%"></i></div>`:'<div class="coming-soon">Контент готовится</div>'}</div>${ready?topicStatus(t.id):'<span class="pill">Недоступно</span>'}</div>`}).join('')}</div>`);document.getElementById('back').onclick=()=>{currentSubjectId=sec.subjectId;page='subject';render()};document.querySelectorAll('[data-topic]').forEach(x=>x.onclick=()=>openTopic(x.dataset.topic))}
...
function topicPage(){
  ...
  shell(`<div class="breadcrumb">Знания → ${esc(s?.name||'')} → ${esc(sec?.name||'')} → ${esc(t.name)}</div><div class="row between"><button class="btn ghost" id="back">← Раздел</button><button class="btn secondary" id="favBtn">${fav?'★':'☆'} Избранное</button></div><div class="card" style="margin-top:12px"><h1>${esc(t.name)}</h1><p class="muted">${esc(t.summary||'')}</p><div class="row"><span class="pill">Анатомия</span>${topicStatus(t.id)}<span class="pill">Проверка: ${esc(uiVerification(t.medicalVerificationStatus))}</span></div><div class="progress" style="margin-top:12px"><i style="width:${Number(p.lastScore||0)}%"></i></div></div>
  <div class="card content-block"><h3>Учебные цели</h3><ol>${(t.learningObjectives||[]).map(o=>`<li>${esc(o.text)}</li>`).join('')}</ol></div>
  ${blocks.map(b=>`<div class="card content-block content-${esc(b.type)}"><span class="pill">${esc(uiBlockType(b.type))}</span><h3>${esc(b.title)}</h3>${renderBlockContent(b)}</div>`).join('')}
  <div class="card content-block"><h3>Источник и верификация</h3><div class="source"><b>${esc(source?.title||'Источник')}</b><div class="muted">${esc(source?.purpose||'')}</div><div class="row" style="margin-top:8px"><span class="pill">${esc(source?.medicalAuthorityLevel||'')}</span><span class="pill ok">${esc(uiVerification(source?.verificationStatus))}</span><span class="pill">Проверено: ${esc(source?.verifiedAt||'')}</span></div></div></div>
...
function testPage(){
  ...
  shell(`${testSession.isReview?`<div class="review-nav"><button class="btn ghost" id="reviewBack">← К повторению</button><button class="btn secondary" id="reviewExit">Выйти</button></div>`:''}<div class="breadcrumb">${testSession.isReview?'Повторение':'Тест темы'} • ${testSession.index+1}/${testSession.questions.length}</div><div class="card question"><h2>${esc(q.stem)}</h2>${q.options.map((o,i)=>`<button class="btn ghost option ${ans?.selected===i?'selected':''}" data-option="${i}" ${ans?'disabled':''}>${esc(o)}</button>`).join('')}<div id="feedback">${ans?`<div class="notice ${ans.correct?'ok':'warn'}">${esc(q.explanation)}</div>`:''}</div>${ans?`<button class="btn" id="nextQ">${testSession.index===testSession.questions.length-1?'Завершить':'Следующий вопрос'}</button>`:''}</div>`);
  if(testSession.isReview){const exit=()=>{testSession=null;page='review';render()};document.getElementById('reviewBack').onclick=exit;document.getElementById('reviewExit').onclick=exit;}
...
function reviewPage(){const rs=Object.values(state.learningData.reviews).filter(r=>r.status!=='COMPLETED');shell(`<h1>Повторение</h1><p class="muted">Слабые темы и запланированные повторы.</p><div class="list">${rs.length?rs.map(r=>{const t=topicBy(r.topicId),ready=isTopicReady(t);return `<div class="item ${ready?'':'disabled-topic'}" ${ready?`data-review="${r.topicId}"`:''}><div><b>${esc(t?.name||r.topicId)}</b><div class="muted">${ready?(r.reviewMode==='FULL_TOPIC'&&!r.weakQuestionIds?.length?'Полный тест темы • из MedWiz 3.1':`${r.weakQuestionIds?.length||0} слабых вопроса`):'Контент готовится'}</div></div><span class="pill weak">${ready?esc(uiReviewStatus(r.status)):'Недоступно'}</span></div>`}).join(''):'<div class="notice">Нет активных повторений.</div>'}</div>`);document.querySelectorAll('[data-review]').forEach(x=>x.onclick=()=>startReview(x.dataset.review))}
function exam(){shell(`<h1>Экзамен</h1><div class="notice">Экзаменационный раздел сохранён без изменений. В этой QA-сборке исправляется только утверждённый вертикальный сценарий пилотной темы; расширение экзамена не выполняется.</div>`)}
function searchPage(){shell(`<h1>Поиск</h1><input id="q" class="search" placeholder="Предмет, раздел или тема"><div id="results" class="list" style="margin-top:12px"></div>`);const q=document.getElementById('q'),out=document.getElementById('results');q.oninput=()=>{const v=q.value.trim().toLowerCase();if(!v){out.innerHTML='';return}const rows=[];subjects().forEach(s=>{if(s.name.toLowerCase().includes(v))rows.push({type:'Предмет',name:s.name,action:`sub:${s.id}`,ready:true})});state.knowledgeData.sections.forEach(s=>{if(s.name.toLowerCase().includes(v))rows.push({type:'Раздел',name:s.name,action:`sec:${s.id}`,ready:true})});state.knowledgeData.topics.forEach(t=>{if((t.name+' '+(t.summary||'')).toLowerCase().includes(v))rows.push({type:'Тема',name:t.name,action:`topic:${t.id}`,ready:isTopicReady(t)})});out.innerHTML=rows.length?rows.map(r=>`<div class="item ${r.ready?'':'disabled-topic'}" ${r.ready?`data-action="${r.action}"`:''}><div><b>${esc(r.name)}</b><div class="muted">${r.type}${r.ready?'':' • Контент готовится'}</div></div><span>${r.ready?'›':'Недоступно'}</span></div>`).join(''):'<div class="notice">Ничего не найдено.</div>';out.querySelectorAll('[data-action]').forEach(el=>el.onclick=()=>{const [kind,id]=el.dataset.action.split(':');if(kind==='topic')openTopic(id);else if(kind==='sub'){currentSubjectId=id;page='subject';render()}else{currentSectionId=id;page='section';render()}})}}

function manage(){shell(`<h1>Управление</h1><div class="tabs"><button class="btn secondary" id="constructor">Конструктор</button><button class="btn secondary" id="favorites">Избранное</button><button class="btn secondary" id="notes">Заметки</button><button class="btn secondary" id="archive">Архив / Корзина</button><button class="btn secondary" id="data">Резервная копия</button></div><div id="manageBody" style="margin-top:14px"></div>`);document.getElementById('constructor').onclick=()=>drawConstructor();document.getElementById('favorites').onclick=()=>drawFavorites();document.getElementById('notes').onclick=()=>drawNotes();document.getElementById('archive').onclick=()=>drawArchive();document.getElementById('data').onclick=()=>drawData();drawConstructor()}
function drawConstructor(){const body=document.getElementById('manageBody');body.innerHTML=`<div class="notice">Конструктор сохраняет стабильные идентификаторы. В этой QA-сборке доступно скрытие/архивирование предметов без удаления учебного контента пилотной темы.</div><div class="list" style="margin-top:12px">${subjects().map(s=>`<div class="item"><div><b>${esc(s.name)}</b><div class="muted">${esc(uiObjectStatus(s.status))}</div></div><div class="row"><button class="btn ghost" data-hide="${s.id}">${s.status==='hidden'?'Показать':'Скрыть'}</button><button class="btn ghost" data-arch="${s.id}">${s.status==='archived'?'Вернуть':'Архив'}</button></div></div>`).join('')}</div>`;body.querySelectorAll('[data-hide]').forEach(b=>b.onclick=()=>{let s=subjectBy(b.dataset.hide);s.status=s.status==='hidden'?'active':'hidden';s.version=(s.version||1)+1;save();drawConstructor()});body.querySelectorAll('[data-arch]').forEach(b=>b.onclick=()=>{let s=subjectBy(b.dataset.arch);s.status=s.status==='archived'?'active':'archived';s.version=(s.version||1)+1;save();drawConstructor()})}
...
function drawNotes(){const body=document.getElementById('manageBody');const rows=Object.entries(state.learningData.notes).filter(([,v])=>String(v||'').trim()).map(([id,text])=>({topic:topicBy(id),text})).filter(x=>x.topic);body.innerHTML=rows.length?`<div class="list">${rows.map(x=>`<div class="card note-card"><div class="row between"><b>${esc(x.topic.name)}</b><button class="btn ghost" data-notetopic="${x.topic.id}">Открыть тему</button></div><p>${esc(x.text)}</p></div>`).join('')}</div>`:'<div class="notice">Сохранённых заметок пока нет.</div>';body.querySelectorAll('[data-notetopic]').forEach(x=>x.onclick=()=>openTopic(x.dataset.notetopic))}
function favoritesPage(){const list=state.learningData.favorites.map(topicBy).filter(Boolean);shell(`<div class="row between"><h1>Избранное</h1><button class="btn ghost" id="favHome">← Главная</button></div><p class="muted">Темы, которые вы отметили для быстрого доступа.</p><div class="list">${list.length?list.map(t=>`<div class="item ${isTopicReady(t)?'':'disabled-topic'}" ${isTopicReady(t)?`data-fav-open="${t.id}"`:''}><div><b>${esc(t.name)}</b>${isTopicReady(t)?'':'<div class="coming-soon">Контент готовится</div>'}</div><span>${isTopicReady(t)?'›':'Недоступно'}</span></div>`).join(''):'<div class="notice">Избранное пусто.</div>'}</div>`);document.getElementById('favHome').onclick=()=>{page='home';render()};document.querySelectorAll('[data-fav-open]').forEach(x=>x.onclick=()=>openTopic(x.dataset.favOpen))}
function notesPage(){const rows=Object.entries(state.learningData.notes).filter(([,v])=>String(v||'').trim()).map(([id,text])=>({topic:topicBy(id),text})).filter(x=>x.topic);shell(`<div class="row between"><h1>Заметки</h1><button class="btn ghost" id="notesHome">← Главная</button></div><p class="muted">Все личные заметки по темам в одном месте.</p><div class="list">${rows.length?rows.map(x=>`<div class="card note-card"><div class="row between"><b>${esc(x.topic.name)}</b>${isTopicReady(x.topic)?`<button class="btn ghost" data-note-open="${x.topic.id}">Открыть тему</button>`:'<span class="pill">Контент готовится</span>'}</div><p>${esc(x.text)}</p></div>`).join(''):'<div class="notice">Сохранённых заметок пока нет.</div>'}</div>`);document.getElementById('notesHome').onclick=()=>{page='home';render()};document.querySelectorAll('[data-note-open]').forEach(x=>x.onclick=()=>openTopic(x.dataset.noteOpen))}
